...
class RegisterViewController: UIViewController {
    ...
    private let fristNameField : UITextField = {
    ...
    override func viewDidLoad() {
        ...
        scrollView.addSubview(fristNameField)
    ...
    override func viewDidLayoutSubviews() {
        ...
        fristNameField.frame = CGRect(x:30,
                                  y:imageView.bottom+10,
                                  width: scrollView.width-60,
                                  height: 52)
        lastNameField.frame = CGRect(x:30,
                                  y:fristNameField.bottom+10,
                                  width: scrollView.width-60,
                                  height: 52)
    ...
    @objc private func registerButtonTapped(){
        ...
        fristNameField.resignFirstResponder()
        lastNameField.resignFirstResponder()
        
        guard let fristName = fristNameField.text,
        ...
        DatabaseManager.shared.userExists(with: email, completion: {exists in
            ...
            guard !exists else {
                //user already exists
                self.alerUserLoginError(message: "Looks like a user account for that email address already exists.")
                return
            }
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authResult: AuthDataResult?, error: Error?) in
            if let error = error {
                ...
                DatabaseManager.shared.insertUser(with: ChatAppUser(firstName: fristName,
                                                                    lastName: lastName,
                                                                    emailAddress: email))
                print("Done :)")
